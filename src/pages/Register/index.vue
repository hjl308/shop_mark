  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <!-- 使用vee-validate插件校验 -->
        <input 
          placeholder="请输入你的手机号" v-model="phone" 
          name="phone" 
      ...
      <div class="content">
        <label>登录密码:</label>
         ...
         <input 
          ...
          name="password" 
          v-validate="{required:true,regex:/^1\w{5,20}$/}" 
      ...
      <div class="content">
        <label>确认密码:</label>
          <!-- 使用vee-validate插件校验 -->
        <input 
          placeholder="请输入你的登录密码" v-model="password1" 
          name="password1" 
      ...
      <div class="controls">
       ...
       <!-- 使用vee-validate插件校验 -->
        <input 
          ...
          v-model="agree" 
          name="agree" 
          v-validate="{required:true,'agreed':true}" 
    ...
    // 获取验证码
      async getCode(){
        try {
          ...
          // 将组件的code值变为仓库的验证码(验证码自动填写了)
          this.code = this.$store.state.user.code
        } catch (error) {}
      },
      // 用户注册
     async userRegister(){
      // 全部表单格式验证结果（返回boolen值）
       const success = await this.$validator.validateAll()
      //  全部表单验证正确才发送请求，进行用户注册
      // 只要有一个表单验证失败，则不能发送请求
      if(success){
        try {
          //  如果成功，路由跳转到登录页面
          const {phone,code,password} = this;
          // 如果都有phone,code,password,password1，就发送请求注册
