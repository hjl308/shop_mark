        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 发送请求，把购买的产品信息通过请求形式通知服务器，服务器进行相应存储 -->
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // 简化，给子组件的数组
      skuImageList(){
        // 如果服务器的数据没回来，skuInfo这个对象是空对象
        return this.skuInfo.skuImageList || []
      },
      // 简化售卖属性列表
      skuSaleAttrValueList(){
        //如果服务器的数据没回来，skuInfo是空对象,那么skuSaleAttrValueList是undefined
    ...
    methods: {
      // 表单元素修改产品个数
      changeSkuNum(event){
        // 用户输入的文本*1,结果NaN或者是数字
        let value = event.target.value*1
        // 如果用户输入非法数字，即出现NaN或者小于1
      ...
      // 加入购物车的回调函数
      async addShopCart(){
        // 1，点击加入购物车按钮，首先将参数带给服务器，发送请求，通知服务器加入购物车的产品是谁
        // 2，知道请求失败或成功。如果成功，进行路由跳转。如果失败，要给用户提示。
        try{
          // 成功
          // 由于仓库使用的是async返回的必是promise对象
         await this.$store.dispatch('detail/addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
        // 路由跳转,并把产品个数信息带给下一级路由
        this.$router.push({ name:'addcartsuccess',query:{ skuNum:this.skuNum}})
        // skuInfo信息比较复杂，采用会话存储（会话结束即消失，HTML5新增的）。采用query传递，地址栏会比较丑
        // 本地存储|对话存储，一般存储的是字符串
